#include "../src/faster_compressed_seq.h"

#define DEBUG
#include "../src/debug.h"
#include <stdlib.h>

static inline void print_values(faster_compressed_seq_t * cs, cmph_uint32 idx)
{
	register cmph_uint32 index;
	
	index = faster_compressed_seq_query(cs, idx);
...
static inline void print_values_packed(char * cs_packed, cmph_uint32 idx)
{
	...
	index = faster_compressed_seq_query_packed(cs_packed, idx);
	fprintf(stderr, "Index[%u]\t= %u\n", idx, index);
}
...
int main(int argc, char **argv)
{
	...
	faster_compressed_seq_init(&cs);
	faster_compressed_seq_generate(&cs, keys_vec, n);
	fprintf(stderr, "Space usage = %u\n", faster_compressed_seq_get_space_usage(&cs));
	for(i = 0; i < n; i++)
	{
		print_values(&cs, i);
	}
	...
	faster_compressed_seq_destroy(&cs);
	fprintf(stderr, "Loading compressed seq structure\n");
	
	faster_compressed_seq_load(&cs, buf, buflen);
	for(i = 0; i < n; i++)
	{
		print_values(&cs, i);
	}
	...
	cs_pack_size = faster_compressed_seq_packed_size(&cs);
	
	cs_packed = (char *) calloc(cs_pack_size, sizeof(char));
	faster_compressed_seq_pack(&cs, cs_packed);
	faster_compressed_seq_destroy(&cs);
