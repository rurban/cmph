...
cmph_t *bmz8_new(cmph_config_t *mph, double c)
{
	...
	if (c == 0)
	    c = 1.15; // validating restrictions over parameter c.
	DEBUGP("c: %f\n", c);
	bmz8->m = (cmph_uint8) mph->key_source->nkeys;
	bmz8->n = (cmph_uint8) ceil(c * bmz8->m);
	...
	if (mph->do_ordering_table) {
	    o = (cmph_uint32 *)calloc((size_t)bmz8->m, sizeof(cmph_uint32));
	    assert(o);
	}
	do
	{
	  ...
	  while(1)
	  {
		...
		bmz8->hashes[0] = hash_state_new(mph->hashfuncs[0], bmz8->n);
		DEBUGP("hash function 2\n");
		bmz8->hashes[1] = hash_state_new(mph->hashfuncs[1], bmz8->n);
		...
		if (!ok)
		{
			--iterations;
			if (o)
			    memset(o, 0, (size_t)bmz8->m * sizeof(cmph_uint32));
			hash_state_destroy(bmz8->hashes[0]);
			bmz8->hashes[0] = NULL;
			hash_state_destroy(bmz8->hashes[1]);
			bmz8->hashes[1] = NULL;
	...
	if (o) {
	    DEBUGP("Ordering table (WIP)\n");
	    for (i = 0; i < bmz8->m; ++i) // all edges
	    {
		cmph_uint32 h1 = graph_vertex_id(bmz8->graph, i, 0);
		cmph_uint32 h2 = graph_vertex_id(bmz8->graph, i, 1);
		if (h1 == h2 && ++h2 > bmz8->n) h2 = 0;
		//FIXME
		//assert(bmz8->g[h1] + bmz8->g[h2] < bmz8->m);
		//o[bmz8->g[h1] + bmz8->g[h2]] = i;
	    }
	}
	graph_destroy(bmz8->graph);
	bmz8->graph = NULL;
               	        ...
               	        if (graph_node_is_critical(bmz8->graph, u) && (!GETBIT(visited,u)))
			{
			        ...
			        while(collision) // lookahead to resolve collisions
				{
					...
					if(collision && (next_g > *biggest_g_value)) // saving the current g value stored in next_g.
					{
						...
						{
							cmph_uint8 *new_unused_g_values = (cmph_uint8*)realloc(unused_g_values, ((size_t)(unused_g_values_capacity + BUFSIZ))*sizeof(cmph_uint8));
							if (!new_unused_g_values) {
							    vqueue_destroy(q);
							    free(unused_g_values);
							    return -1;
							}
							else
							    unused_g_values = new_unused_g_values;
...
int bmz8_dump(cmph_t *mphf, FILE *fd)
{
	...
	CHK_FWRITE(&(data->m), sizeof(cmph_uint8), (size_t)1, fd);

	CHK_FWRITE(data->g, sizeof(cmph_uint8)*(data->n), (size_t)1, fd);
#ifdef DEBUG
	fprintf(stderr, "G: ");
...
void bmz8_load(FILE *f, cmph_t *mphf)
{
	...
	CHK_FREAD(&(bmz8->n), sizeof(cmph_uint8), (size_t)1, f);
...
cmph_uint8 bmz8_search_packed(void *packed_mphf, const char *key, cmph_uint32 keylen)
{
	...
	cmph_uint8 *g_ptr = h2_ptr + hash_state_packed_size(h2_type);

	cmph_uint8 n = *g_ptr++;

	cmph_uint8 h1 = (cmph_uint8)(hash_packed(h1_ptr, h1_type, key, keylen) % n);
	cmph_uint8 h2 = (cmph_uint8)(hash_packed(h2_ptr, h2_type, key, keylen) % n);
	DEBUGP("key: %s h1: %u h2: %u\n", key, h1, h2);
	if (h1 == h2 && ++h2 > n) h2 = 0;
