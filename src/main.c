...
{
	cmph_uint32 i;
	fprintf(stderr, "usage: %s [-v][-h][-V][-C][-r][-O] [-k nkeys] [-f hash_function] [-g [-c algorithm_dependent_value][-s seed] ] [-a algorithm] [-M memory_in_MB] [-b algorithm_dependent_value] [-t keys_per_bin] [-d tmp_dir] [-m file.mph] [-o c_file] [-p c_func_prefix] keysfile\n", prg);
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		char ch = (char)getopt(argc, argv, "hVvgCrOc:k:a:M:b:t:f:m:d:s:o:p:");
		if (ch == -1) break;
		switch (ch)
		{
			case 's':
