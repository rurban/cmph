...
cmph_t *bmz_new(cmph_config_t *mph, double c)
{
	...
	do
	{
	  ...
	  for (i = 0; i < bmz->n; ++i) // critical nodes
	  {
		...
		{
		    if(c > 1.14)
			restart_mapping = bmz_traverse_critical_nodes(bmz, i,
					      &biggest_g_value, &biggest_edge_value, used_edges, visited);
		    else
			restart_mapping = bmz_traverse_critical_nodes_heuristic(bmz, i,
					      &biggest_g_value, &biggest_edge_value, used_edges, visited);
		    if(restart_mapping)
			break;
		}
	...
	mphf = (cmph_t *)xcalloc(sizeof(cmph_t), 1);
	if (mph->do_ordering_table) {
	    compressed_seq_t co;
	    mph->ordering_table = (cmph_uint32 *)xcalloc((size_t)bmz->m, sizeof(cmph_uint32));
	    if (mph->verbosity)
	    ...
	    compressed_seq_generate(&co, mph->ordering_table, bmz->m);
	...
	if (mph->ordering_table) {
	    fprintf(stderr, "O: ");
	    for (i = 0; i < bmz->m; ++i) fprintf(stderr, "%u ", mph->ordering_table[i]);
	    fprintf(stderr, "\n");
	}
#endif
               	        ...
               	        if (graph_node_is_critical(bmz->graph, u) && (!GETBIT(visited,u)))
			{
			        ...
			        while(collision) // lookahead to resolve collisions
				{
					...
					if(collision && (next_g > *biggest_g_value)) // saving the current g value stored in next_g.
					{
						...
						{
						        cmph_uint32 *new_unused_g_values = (cmph_uint32 *)realloc(unused_g_values, (unused_g_values_capacity + BUFSIZ)*sizeof(cmph_uint32));
...
int bmz_dump(cmph_t *mphf, FILE *fd)
{
	...
	CHK_FWRITE(data->g, sizeof(cmph_uint32)*(data->n), (size_t)1, fd);
#ifdef DEBUG
	cmph_uint32 i;
	fprintf(stderr, "G: ");
	...
	if (mphf->packed_co_size) {
	    DEBUGP("Dumping packed ordering table with %u entries\n", data->m);
	    CHK_FWRITE(&(mphf->packed_co_size), sizeof(cmph_uint32), (size_t)1, fd);
	    CHK_FWRITE(mphf->packed_co, (size_t)mphf->packed_co_size, (size_t)1, fd);
	}
	return 1;
}
