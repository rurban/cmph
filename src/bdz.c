...
cmph_t *bdz_new(cmph_config_t *mph, double c)
{
	...
	if (mph->do_ordering_table) {
		o = (cmph_uint32 *)malloc(bdz->m * sizeof(cmph_uint32));
		assert(o);
		DEBUGP("Create ordering table (WIP)\n");
...
int bdz_compile(cmph_t *mphf, cmph_config_t *mph, FILE *out)
{
	...
	fprintf(out, "};\n");
	if (mphf->o) {
		fprintf(out, "    const uint8_t ordering_table[%u] = {\n        ", bdz->m);
		for (unsigned i=0; i < bdz->m; i++) {
			fprintf(out, "%u, ", mphf->o[i]);
			if (i % 16 == 15)
				fprintf(out, "\n        ");
		}
